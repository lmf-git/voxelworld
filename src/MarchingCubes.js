...
export class MarchingCubes {
    constructor() {
        ...
        this.triTable = this.generateTriTable();
    }
    ...
    interpolateVertex(isolevel, p1, p2, v1, v2) {
        if (Math.abs(isolevel - v1) < 0.00001) return p1;
        if (Math.abs(isolevel - v2) < 0.00001) return p2;
    ...
    polygonise(cube, isolevel) {
        ...
        if (this.edgeTable[cubeindex] === 0) return [];

        if (this.edgeTable[cubeindex] & 1) vertList[0] = this.interpolateVertex(isolevel, cube.p[0], cube.p[1], cube.val[0], cube.val[1]);
        if (this.edgeTable[cubeindex] & 2) vertList[1] = this.interpolateVertex(isolevel, cube.p[1], cube.p[2], cube.val[1], cube.val[2]);
        if (this.edgeTable[cubeindex] & 4) vertList[2] = this.interpolateVertex(isolevel, cube.p[2], cube.p[3], cube.val[2], cube.val[3]);
        if (this.edgeTable[cubeindex] & 8) vertList[3] = this.interpolateVertex(isolevel, cube.p[3], cube.p[0], cube.val[3], cube.val[0]);
        if (this.edgeTable[cubeindex] & 16) vertList[4] = this.interpolateVertex(isolevel, cube.p[4], cube.p[5], cube.val[4], cube.val[5]);
        if (this.edgeTable[cubeindex] & 32) vertList[5] = this.interpolateVertex(isolevel, cube.p[5], cube.p[6], cube.val[5], cube.val[6]);
        if (this.edgeTable[cubeindex] & 64) vertList[6] = this.interpolateVertex(isolevel, cube.p[6], cube.p[7], cube.val[6], cube.val[7]);
        if (this.edgeTable[cubeindex] & 128) vertList[7] = this.interpolateVertex(isolevel, cube.p[7], cube.p[4], cube.val[7], cube.val[4]);
        if (this.edgeTable[cubeindex] & 256) vertList[8] = this.interpolateVertex(isolevel, cube.p[0], cube.p[4], cube.val[0], cube.val[4]);
        if (this.edgeTable[cubeindex] & 512) vertList[9] = this.interpolateVertex(isolevel, cube.p[1], cube.p[5], cube.val[1], cube.val[5]);
        if (this.edgeTable[cubeindex] & 1024) vertList[10] = this.interpolateVertex(isolevel, cube.p[2], cube.p[6], cube.val[2], cube.val[6]);
        if (this.edgeTable[cubeindex] & 2048) vertList[11] = this.interpolateVertex(isolevel, cube.p[3], cube.p[7], cube.val[3], cube.val[7]);

        const triangles = [];
        const triTable = this.triTable[cubeindex];

        for (let i = 0; i < triTable.length; i += 3) {
            triangles.push(vertList[triTable[i]]);
            triangles.push(vertList[triTable[i + 1]]);
            triangles.push(vertList[triTable[i + 2]]);
        }
