...
## Marching Cubes implementation for voxel mesh generation

var edge_table: Array[int]
var tri_table: Array

func _init() -> void:
	_initialize_edge_table()
	_initialize_tri_table()

func _initialize_edge_table() -> void:
	edge_table = [
...
func _initialize_tri_table() -> void:
	tri_table = [
		...
		[7, 11, 6], [7, 6, 11], [3, 0, 8, 11, 7, 6], [0, 1, 9, 11, 7, 6],
		...
		[6, 8, 4, 11, 8, 6], [3, 6, 11, 3, 0, 6, 0, 4, 6],
		...
		[0, 3, 8, 5, 6, 10], [10, 5, 6], [11, 5, 10, 7, 5, 11],
		...
		[9, 10, 2, 0, 9, 2], [2, 3, 8, 2, 8, 10, 0, 1, 8, 1, 10, 8],
		[1, 10, 2], [1, 3, 8, 9, 1, 8], [0, 9, 1], [0, 3, 8], []
	]

func interpolate_vertex(isolevel: float, p1: Vector3, p2: Vector3, v1: float, v2: float) -> Vector3:
	...
	var mu: float = (isolevel - v1) / (v2 - v1)
	return p1.lerp(p2, mu)

func polygonise(cube_points: Array[Vector3], cube_values: Array[float], isolevel: float) -> Array[Vector3]:
	var vert_list: Array[Vector3] = []
	vert_list.resize(12)

	var cube_index: int = 0

	if cube_values[0] < isolevel: cube_index |= 1
	if cube_values[1] < isolevel: cube_index |= 2
	if cube_values[2] < isolevel: cube_index |= 4
	if cube_values[3] < isolevel: cube_index |= 8
	if cube_values[4] < isolevel: cube_index |= 16
	if cube_values[5] < isolevel: cube_index |= 32
	if cube_values[6] < isolevel: cube_index |= 64
	if cube_values[7] < isolevel: cube_index |= 128

	if edge_table[cube_index] == 0:
		return []

	if edge_table[cube_index] & 1: vert_list[0] = interpolate_vertex(isolevel, cube_points[0], cube_points[1], cube_values[0], cube_values[1])
	if edge_table[cube_index] & 2: vert_list[1] = interpolate_vertex(isolevel, cube_points[1], cube_points[2], cube_values[1], cube_values[2])
	if edge_table[cube_index] & 4: vert_list[2] = interpolate_vertex(isolevel, cube_points[2], cube_points[3], cube_values[2], cube_values[3])
	if edge_table[cube_index] & 8: vert_list[3] = interpolate_vertex(isolevel, cube_points[3], cube_points[0], cube_values[3], cube_values[0])
	if edge_table[cube_index] & 16: vert_list[4] = interpolate_vertex(isolevel, cube_points[4], cube_points[5], cube_values[4], cube_values[5])
	if edge_table[cube_index] & 32: vert_list[5] = interpolate_vertex(isolevel, cube_points[5], cube_points[6], cube_values[5], cube_values[6])
	if edge_table[cube_index] & 64: vert_list[6] = interpolate_vertex(isolevel, cube_points[6], cube_points[7], cube_values[6], cube_values[7])
	if edge_table[cube_index] & 128: vert_list[7] = interpolate_vertex(isolevel, cube_points[7], cube_points[4], cube_values[7], cube_values[4])
	if edge_table[cube_index] & 256: vert_list[8] = interpolate_vertex(isolevel, cube_points[0], cube_points[4], cube_values[0], cube_values[4])
	if edge_table[cube_index] & 512: vert_list[9] = interpolate_vertex(isolevel, cube_points[1], cube_points[5], cube_values[1], cube_values[5])
	if edge_table[cube_index] & 1024: vert_list[10] = interpolate_vertex(isolevel, cube_points[2], cube_points[6], cube_values[2], cube_values[6])
	if edge_table[cube_index] & 2048: vert_list[11] = interpolate_vertex(isolevel, cube_points[3], cube_points[7], cube_values[3], cube_values[7])

	var triangles: Array[Vector3] = []
	var tri_indices: Array = tri_table[cube_index]

	for i in range(0, tri_indices.size(), 3):
		if i + 2 < tri_indices.size():
			triangles.append(vert_list[tri_indices[i]])
			triangles.append(vert_list[tri_indices[i + 1]])
			triangles.append(vert_list[tri_indices[i + 2]])
